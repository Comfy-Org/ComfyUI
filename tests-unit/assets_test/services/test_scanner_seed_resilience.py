...
def test_seed_persists_remaining_specs_when_path_vanishes_during_recovery_hash(
    ...
    with patch("app.assets.scanner.mode.hashing_enabled", return_value=True):
        created, _, error = seed_with_recovery(session, specs)
    session.commit()
...
def test_seed_persists_fresh_stat_after_spec_was_built(
    ...
    created, error = seed_asset_specs(session, [spec], stat_seed_specs([spec]))
    session.commit()
...
def test_seed_takes_every_stat_before_the_write_transaction(
    ...
    with patch("app.assets.scanner.mode.hashing_enabled", return_value=True):
        created, _ = scanner.insert_asset_specs(specs, set())
