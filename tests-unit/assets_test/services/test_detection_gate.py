...
from app.assets.scanner_changes import is_path_under_prefixes, queue_pending_verification
...
def test_hash_mode_real_edit_splits(session, temp_dir: Path, monkeypatch):
    ...
    def traced_live(candidate_session, candidate_prefixes):
        ...
        trace_lines.append(
            ...
            + repr(
                [
                    (row.id, row.path, row.mtime_ns, row.size_bytes, row.is_missing)
                    for row in rows
                ]
            )
        )
    ...
    def traced_detect(candidate_session, content, stat_result, hashing_is_enabled):
        trace_lines.append(
        ...
        return real_detect(candidate_session, content, stat_result, hashing_is_enabled)
    ...
    drain_rows = [
        (content.id, content.path, content.hash, content.mtime_ns, content.size_bytes, content.is_missing)
        for content in session.scalars(select(AssetContent).order_by(AssetContent.created_at))
    ]
    trace_lines.append(
        f"DRAIN queued_before={queued_before_drain!r} processed={processed!r} "
        f"queued_after={queued_after_drain!r} rows={drain_rows!r}"
    )
    trace_blob = "WIN12_DIAG " + "\n".join(trace_lines)
    print("\n" + trace_blob + "\n", file=sys.stdout, flush=True)  # noqa: T201
    print("\n" + trace_blob + "\n", file=sys.stderr, flush=True)  # noqa: T201
    pytest.fail(trace_blob)
