...
def test_single_hash_match_recovers(session, temp_dir: Path):
    ...
    with patch("app.assets.scanner.mode.hashing_enabled", return_value=True):
        created, _, error = seed_with_recovery(session, [_spec(path)])
    session.commit()
...
def test_unstable_hash_requeues(session, temp_dir: Path):
    ...
    with (
        ...
        created, pending, error = seed_with_recovery(session, [_spec(path)])
    session.commit()

    assert error is None
    assert created == 0
    assert pending == [str(path)]
    assert session.get(AssetContent, missing.id).is_missing is True
...
def test_recovery_skips_a_path_a_live_row_already_occupies(session, temp_dir: Path):
    ...
    result = recover_missing_content_from_preparation(
        ...
        str(path),
        stat_result,
        prepare_missing_content_recovery(str(path), stat_result),
        [],
    )
    session.commit()
