...
Every database write goes through `run_write_txn`, which opens a dedicated writer session, takes the write lock up front with `BEGIN IMMEDIATE`, and retries on a locked database within a bounded deadline (see Write pressure and reader starvation). Reader connections are opened `query_only`, so a write attempted through an ordinary read session fails immediately with a read-only-database error rather than becoming a second writer by accident.

Background writes are bounded: no scanner transaction covers more than `MAX_WRITE_BATCH` rows, and the scanner checks for pause and cancellation between batches, so a pause takes effect one batch later instead of at the end of the phase. Seeding, reference reconciliation, pruning, and enrichment all chunk their work to that size; the verification and hash-transition drains already write one entry per transaction.

The batch size is 25. A transaction holds the write lock for its whole duration, so the batch size sets both the stall an urgent writer inherits when it arrives mid-batch and the rollback blast radius when one row in a batch fails.

Chunking matters more than its arithmetic suggests. A commit costs roughly 20ms fixed plus roughly 0.04ms per row, so smaller batches hold the lock for less time each but hold it more often, and the total time spent holding it goes up. What that accounting misses is that a competing writer does not care about the total; it cares whether it can ever acquire. Because the scanner releases and reacquires between chunks, a blocked writer gets a fresh attempt in every gap, and the bounded retry rides over a long series of short holds instead of timing out against one long one. Measured against deliberately slowed storage, writers have waited a full minute across many chunk boundaries and still succeeded, where a single hold of that length would have failed. Chunking turns one fatal wait into many survivable ones.

Write callables never touch the filesystem. Stat, hashing, and metadata extraction run in a preflight outside the transaction, and the closure receives the facts that preflight observed; inside the lock it only reads and writes rows. Two invariants make that safe:

- Pair-write: any statement that sets a content hash also sets the byte size and modification time from the same snapshot, so a stored hash always describes the stat facts stored beside it.
- Row freshness: before mutating a content row from preflight facts, a closure compares the row's current missing state, byte size, and modification time against the values the preflight read from the database — never against the stat the preflight took from disk, because a row that is legitimately behind disk is the normal steady state and comparing against disk would reject valid work. On a mismatch the closure skips that row and leaves it untouched; the next scan re-observes it and repairs it.

A persistently unstable upload is refused rather than hashed under the lock: when its preflight facts keep changing across attempts, the request fails instead of writing content facts and file metadata that describe different bytes.

## Operational limits

### Write pressure and reader starvation

The asset database is SQLite in WAL journal mode. Reader connections use a 30-second busy timeout. Writer connections poll `BEGIN IMMEDIATE` in Python, so each lock wait is capped at 30 seconds while the overall retry deadline remains 60 seconds. Several paths still contend for the single writer lock: a non-deduplicated upload writes its bytes and mints a delivery record, while a deduplicated upload reuses existing content and mints only the record; a same-path write whose hash has changed retires the old content and inserts new content, while a same-path write whose hash matches refreshes the existing record in place; execution outputs register per-emission during the generation loop; a background enrichment pass fills hashes and metadata in bounded batches; hash-serves write access time to every record sharing the served content; and the upload dedup claim mints its record once a preflight has already re-checked the file and extracted its metadata outside the lock.

When a writer holds the lock for longer than the busy timeout, a competing writer waits the timeout out and then fails. That is the failure this system is built to avoid, and it is not specific to a session that reads before it writes: a write with no preceding read fails the same way. Every writing session goes through `run_write_txn`, which opens a dedicated writer session and issues `BEGIN IMMEDIATE` up front. Declaring write intent before any statement runs lets the writer queue for the lock and retry, rather than discovering the conflict partway through a unit of work it would have to abandon. A write that still finds the database locked retries up to 5 attempts total, sleeping 0.05s/0.1s/0.2s/0.4s between attempts (each jittered ±50%) and giving up once 60 seconds have elapsed since the first attempt; only a locked-database `OperationalError` is retried, every other exception propagates immediately. Those 5 attempts and their backoff table are reachable only when the lock error surfaces from inside the callback, after `BEGIN IMMEDIATE` has already succeeded. Contention at `BEGIN IMMEDIATE` itself behaves differently: each attempt polls for up to 30 seconds before yielding, so the 60-second deadline admits about two attempts and the backoff sleeps are negligible beside the polling. Either way the bound is the same 60-second deadline, after which a locked-database `OperationalError` is raised. Contributors must not open a writing session with `create_session()` directly. `run_write_txn` is the only sanctioned path for writes, and `tests-unit/assets_test/test_write_session_convention.py` enforces that automatically.

WAL journal mode leaves `-wal` and `-shm` sidecar files next to the main database file; treat all three as one unit for backups and copies. WAL is not supported on network filesystems, so pointing `--database-url` at a network-mounted path fails at startup with a `RuntimeError` rather than silently falling back to a less safe journal mode.

All of the above applies only when `--enable-assets` is set. The asset system is the database's only writer, so without it there is nothing to serialise: startup binds a single engine that sets `foreign_keys=ON` and nothing else, exactly as it did before this journal mode existed. A default install therefore never promotes journal mode, never leaves `-wal`/`-shm` sidecars, and cannot fail to start on a filesystem that rejects WAL. Anything that adds a non-asset writer must move this gate, not work around it.
