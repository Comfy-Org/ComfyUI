...
Chunking is what lets a writer wait out a busy scanner. The scanner releases the lock between chunks and a blocked writer re-attempts in every gap, so the bounded retry crosses many short holds where a single hold of the same total length would exceed its deadline. Under deliberately slowed storage, writers have blocked for a minute across chunk boundaries and still committed.
