...
The batch size is 25. A transaction holds the write lock for its whole duration, so the batch size sets both the stall an urgent writer inherits when it arrives mid-batch and the rollback blast radius when one row in a batch fails. A commit costs roughly 20ms fixed plus roughly 0.04ms per row, so the batch size trades worst-case hold against total lock occupancy: across a 10,000-file scan, 25-row batches hold the lock about 21ms at a time over 400 commits, where 500-row batches hold it about 39ms over 20. Both sit three orders of magnitude below the point at which a competing writer gives up, so this constant is not what keeps the system correct — moving filesystem work out of the transaction is. 25 is kept for the shorter worst-case hold.
