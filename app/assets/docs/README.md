...
The batch size is 25. A transaction holds the write lock for its whole duration, so the bound is exactly the stall an urgent writer inherits when it arrives mid-batch, and it is also the rollback blast radius when one row in a batch fails. This is a latency bound rather than a throughput optimisation: measured lock-held medians are 20ms for a 25-row seed batch and 27ms for a 25-row enrichment batch, against 3.8ms for the single-row insert an urgent writer is typically waiting to make. Chunking costs aggregate throughput, because each commit pays a fixed cost and twenty small transactions therefore hold the lock longer in total than one large one. That trade is deliberate: the scanner yields between batches, so the total is spread across gaps an urgent writer can take, where a single large batch holds the lock in one block it cannot.
