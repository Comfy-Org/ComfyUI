...
The batch size is 25. A transaction holds the write lock for its whole duration, so the bound is exactly the stall an urgent writer inherits when it arrives mid-batch, and it is also the rollback blast radius when one row in a batch fails. 25 sits just past the point where a larger batch stops buying throughput: measured lock-held medians are 20ms for a 25-row seed batch and 27ms for a 25-row enrichment batch, against a 0.03ms empty-transaction floor, roughly 1ms per row, and 3.8ms for the single-row insert an urgent writer is typically waiting to make.
