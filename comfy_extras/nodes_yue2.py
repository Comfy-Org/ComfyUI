    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input("seed", default=0, min=0, max=0xffffffffffffffff, control_after_generate=True),
                io.Combo.Input("mode", options=["full", "melody"], tooltip="full: generates melody and chords; melody: generates melody only, recommended for covers."),
                io.Float.Input("max_duration", default=360.0, min=0.04, max=960.0, step=0.04, tooltip="Maximum duration; generation can stop earlier. The release uses a 360-second budget."),
