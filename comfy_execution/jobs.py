...
def normalize_history_item(prompt_id: str, history_item: dict, include_outputs: bool = False) -> dict:
    ...
    if status_info:
        has_errors = execution_summary.get('has_errors', bool(execution_errors) or execution_error is not None)
        execution_error_count = execution_summary.get('execution_error_count', len(execution_errors) or int(execution_error is not None))
    else:
        has_errors = None
